...
X_resized = tf.image.resize(X_3ch, (75, 75)).numpy()
...
history=inception_model.fit(X_train, y_train, epochs=50, batch_size=32, validation_split=0.2, callbacks=[early_stop])
...
def predict_heart_disease(user_input):
    ...
    input_resized = tf.image.resize(input_3ch, (75,75)).numpy()
    prediction = inception_model.predict(input_resized)
